...
| 0:30–1:25 | Follow the six-stage **BREACH PROGRESSION** rail while WEB, CODE, IDENTITY, and OBSERVABILITY run. | “The swarm has touched real, harmless tripwires inside this disposable range. The rail advances only when runtime evidence supports RECON, DECEPTION, IDENTITY, PRIVILEGE, PRODUCTION, or CANARY. A decoy hit raises evidence; it does not claim compromise.” |
...
| 1:50–2:40 | Follow the same team composition, tripwires, and denied deployment edge. | “Detection can happen before containment is necessary. Ordinary investigation continues, then FLASH0VER evaluates the sensitive operation with the requesting agent, composed capability chain, and provenance path.” |
| 2:40–3:00 | Keep the swarm visible beside the inline **EMERGENT CAPABILITY BLOCKED** result, then show ACME still HEALTHY. | “The collector is clean and production never changed. Real tripwires fired, but the dangerous composition was contained. The live result shows operational agents, quarantines, and blocked paths from this run, and the final canary remained safe.” |

The deception-grid build completed three consecutive local V2 OFF runs in 72.772–77.244s and three ENFORCE runs in 62.286–69.386s, all with zero model failures or retries. The minimal Tenki check completed OFF in 79.296s and ENFORCE in 60.692s, destroyed both disposable sandboxes, and confirmed orphan count 0. Treat those as evidence, not countdowns. If the model takes longer, narrate only current real events. Do not call a decoy touch a compromise. Announce compromise only after real target change and current-canary receipt; announce protection only after `TARGET_PROTECTED`, unchanged production, and a clean collector.
...
| 2:15–2:42 | Leave the inline containment result and swarm map visible, then press **OFF ↔ ENFORCE**. | “These are two persisted real run IDs. OFF reached the collector. ENFORCE allowed the same ordinary stages but denied the dangerous composition, and the collector remained safe.” |
...
- **Immediately after compromise or a policy block:** Use the normal header **RESET** control. This stops any active work, rotates the canary, clears the collector, and clears the current live view. Range V2 outcomes remain inline, so the graph and controls stay usable.
- **Browser sizing changes:** The in-page presentation layout uses the available tab viewport and never invokes a browser fullscreen API. At 100% zoom it is verified at 917×603 and 1280×720.
