...
Three consecutive local V2 OFF runs completed in 66.923–82.718s; three consecutive ENFORCE runs completed in 65.135–69.906s. Two verified Tenki V2 OFF runs completed in 74.428–90.233s, and two ENFORCE runs completed in 73.758–81.657s. Treat those as evidence, not countdowns. If the model takes longer, continue narrating only current real events. Do not announce compromise until both `TARGET_STATE_CHANGED` and `CANARY_LEAK` exist. Do not announce protection until the run finishes with `TARGET_PROTECTED`, unchanged production, and a clean collector. Local remains the guaranteed venue fallback; use Tenki when preflight reports it READY and network conditions are stable.
