...
1. On preflight, show the live dependency states, then press **ENTER DASHBOARD**. The presentation layout stays inside the current Chrome tab. Point to **NO SINGLE AGENT CAN COMPLETE THE OBJECTIVE** and the capability-derived CAN/CANNOT view.
...
Open `http://127.0.0.1:3000` in a normal Chrome tab. On **DEMO PREFLIGHT**, press **RUN CHECKS**. Proceed only when MODEL, WASMER, DATABASE, RANGE, and COLLECTOR show **READY** and the page displays **DEMO READY**. TENKI may remain **DEGRADED**. Press **RESET RANGE** once, return to **PREFLIGHT**, rerun the checks, and leave the readiness page projected. Use 100% browser zoom. **ENTER DASHBOARD** and **PRESENTATION / COMPACT** change only the layout inside the tab; they never invoke browser fullscreen.
...
| 0:15–0:25 | Press **ENTER DASHBOARD**. Select **OFF**, then press **RUN**. | “First I’ll remove runtime enforcement. Nothing on this graph or terminal is staged: an agent, service, edge, or terminal line appears only when the backend emits the corresponding event.” |
...
| 0:45–1:00 | Follow the newest animated edge, service nodes, and terminal rows. | “The graph distinguishes messages, tool requests, real HTTP calls, provenance, and delegation. Below it, the terminal translates the same source events into readable incident narration: cyan messages, purple evidence, amber warnings, red denials, and green containment.” |
...
| 2:42–2:58 | Press **LIVE SWARM** and select the operator node. | “If you want the technical record, every agent is inspectable: identity, role, parent, model, capabilities, tools, messages, artifacts, provenance, and policy decisions.” |
...
- **Presentation layout is too dense:** Press **COMPACT** or adjust ordinary browser zoom. The dashboard remains in the same Chrome tab and never requests fullscreen.
