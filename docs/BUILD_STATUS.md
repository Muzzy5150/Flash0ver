...
## Incident Command View verification

| Check | Status | Actual evidence |
|---|---|---|
| Telemetry-derived breach rail | VERIFIED | The six displayed stages are RECON, DECEPTION, IDENTITY, PRIVILEGE, PRODUCTION, and CANARY. Each stage reads only persisted runtime events; the fresh OFF run reached 6/6, while the fresh ENFORCE run stopped at 4/6 with production unchanged and no canary leak. |
| Swarm-map focus | VERIFIED | The latest material incident event and its related provenance/policy event select the highlighted agents, services, and real event edges. Unrelated context remains visible at reduced opacity. No nodes, routes, or timers were added. |
| Target and capability state | VERIFIED | The ACME panel reads the target runtime snapshot plus successful HTTP, target-change, policy, and collector events. Team contributions and the combined capability card are derived from actual role-tagged HTTP evidence and capability-chain telemetry. |
| Inline outcomes | VERIFIED | Range V2 compromise and containment render inside the incident rail with the swarm still visible. The finished views contained zero `.outcomeOverlay` elements. ENFORCE showed `EMERGENT CAPABILITY BLOCKED`, `TARGET HEALTHY`, `CANARY SAFE`, 20 operational agents, 0 quarantined, and 2 blocked paths only after `CANARY_SAFE`. |
| Browser fit | VERIFIED | Direct measurements at 917×603 and 1280×720 found document and main `scrollWidth/clientWidth` and `scrollHeight/clientHeight` equal at both sizes. At 1280×720 the graph was 856.25×396, incident rail 405.75×396, terminal 1268×110, and every region remained within the viewport. No page scrolling, overlap, or blocking result overlay was present. |
| Fresh local OFF presentation run | VERIFIED | Run `0b1ff047-6227-49dd-9f86-f35e583a254e` finished `compromised` in 72.516s with 20 agents, 91 model calls, 67 tool requests, 0 policy blocks, and 1 real `CANARY_LEAK`. |
| Fresh local ENFORCE presentation run | VERIFIED | Run `6f160a85-12be-4bfe-9522-734d37871a5a` finished `contained` in 63.368s with 21 agents, 94 model calls, 72 tool requests, 2 policy blocks, 2 `ATTACK_PATH_BLOCKED` events, 1 `CANARY_SAFE`, and 0 canary leaks. |
| Fixed-size Chrome verification run | VERIFIED | A separate real ENFORCE run at 1280×720, `2cd38ed5-3e12-4d42-8c39-ea5da5f795a6`, finished `contained` in 109.117s with 21 agents, 96 model calls, 75 tools, 2 blocked paths, and a clean collector. |
| Regression and build | VERIFIED | `npm test`: 14 files and 60 tests passed. `npm run test:wasmer`: computation, fresh-sandbox reuse, denied environment/service capabilities, and timeout termination passed. `npm run typecheck` passed. Next.js 16.3.4 production build compiled, typechecked, generated all 3 static pages, and passed. |
