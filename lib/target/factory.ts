...
import type { RangeVersion,TargetRuntime } from './types';
...
export function targetFromEnv(env:NodeJS.ProcessEnv=process.env,bus?:EventBus):TargetSelection {
 ...
 if(requested==='tenki')return {runtime:new TenkiTargetRuntime({apiKey:env.TENKI_API_KEY||'',version:range as RangeVersion,bus}),requested};
