...
interface ReliabilityRow{mode:Mode;attempt:number;runId:string;outcome:string;durationMs:number;modelFailures:number;retries:number;agents:number;modelCalls:number;tools:number;http:number;warnings:number;blocks:number;collector:'COMPROMISED'|'SAFE'}
