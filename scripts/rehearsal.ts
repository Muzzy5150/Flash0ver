...
try{
 ...
 const checks=await runPreflight({provider:providerFromEnv(),bus,target,wasmer,probeModel:true});
 ...
 const blockers=checks.filter(check=>mandatory.has(check.name)&&check.status!=='READY');
...
}catch(error){console.error(`DEMO BLOCKED · ${error instanceof Error?error.message:'rehearsal failed'}`);process.exitCode=2;}finally{await wasmer.stop();await target.stop();bus.close();}
