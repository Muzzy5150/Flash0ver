  ...
  for(const service of ACME_SERVICES){
   const server=createServer(async(req,res)=>{
    ...
    try{
     ...
     if(service==='observability'&&url.pathname==='/logs/production'&&req.method==='GET')return send(200,{environment:'acme-prod-usw2',channel:this.channel,currentRelease:this.target.releaseId,status:this.target.systemStatus});
